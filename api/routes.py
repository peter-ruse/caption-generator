...
async def generate_caption(request: GenerateCaptionRequest):
    ...
    text_generator = service.generate_caption(
        request.text, request.caption_style, request.social_media_platform  # type: ignore
    )
    caption = "".join(chunk for chunk in text_generator)
    caption = f"{caption}\n\nFor more details, please visit {request.link}"

    return HTMLResponse(
        f"""
        <div id="caption-text" class="text-gray-800 leading-relaxed whitespace-pre-wrap fade-in">{caption}</div>
